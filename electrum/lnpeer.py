            ...
            for next_chan in next_peer.channels.values():
                ...
                async def wrapped_callback():
                    ...
                    try:
                        ...
                        e = OnionRoutingFailure(code=OnionFailureCode.TEMPORARY_NODE_FAILURE, data=b'')
                        self.jit_failures[next_chan_scid.hex()] = e
    ...
    async def maybe_forward_trampoline(
        ...
        if next_peer and next_peer.accepts_zeroconf():
            ...
            for next_chan in next_peer.channels.values():
                ...
                next_onion, amount_msat, cltv, session_key = self.create_onion_for_route(
                    route=route,
                    amount_msat=amt_to_forward,
                    total_msat=amt_to_forward,
                    payment_hash=payment_hash,
                    min_final_cltv_expiry=trampoline_cltv_delta,
                    payment_secret=payment_secret,
